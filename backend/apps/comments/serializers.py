...
class CommentCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_post(self, value):
        if not Post.objects.filter(id=value.id, status='published').exists:
            raise serializers.ValidationError('Post notm found')
        return value
...
class CommentUpdateSerializer(serializers.ModelSerializer):
    '''Сериализатор для обновление комментариев'''
...
class CommentDetailSerializer(CommentSerializers):
    '''Детальный сериализатр комментария с ответами'''
    replies = serializers.SerializerMethodField()

    class Meta(CommentSerializers.Meta):
        fields = CommentSerializers.Meta.fields + ['replies']
