...
class PaymentCreateSerializer(serializers.Serializer):
    '''Сериализатор для создания платежки'''
    ...
    def validate(self, attrs):
        ...
        # Проверка на наличие ожилающих платежей
        ...
        if pending_payments:
            raise serializers.ValidationError({
                'non_field_erros': ['User has pending payments. Please complete or cancel them first']
            })
        return attrs
...
class RefundSeriaizer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'status', 'created_by', 'processed_at', 
        ]
    
    def get_payment_info(self, obj):
        '''Возврат инфы о платеже'''
        return {
            'id': obj.payment.id,
    ...
    def validate(self, attrs):
        ...
        if payment_id:
            ...
            # Проверка, что сумма вощврата не превышает сумму платежа
            total_refunded = payment.refunds.filter(
                status='succeeded'
            ).aggregate(total=models.Sum('amoun'))['total'] or Decimal(0)
