...
class PaymentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        '''Возврат платежей текущего юзера'''
        ...
        try:
            with transaction.atomic():
                ...
                success_url = serializer.validated_data.get(
                    'success_url',
                    f'{settings.FRONTEND_URL}/paymentsuccess?session_id={{CHECKOUT_SESSION_ID}}'
                )
...
def payment_status(request, payment_id):
    ...
    try:
        ...
        # Если есть session_id, проверка статуса в Stripe
        ...
        if payment.is_successfully and payment.subscription:
...
class RefundListView(generics.ListAPIView):
    '''Список возврата для администраторов'''
    serializer_class = RefundSeriaizer
        ...
        if serializer.is_valid():
            ...
            with transaction.atomic():
                ...
                if success:
                    ...
                    response_serializer = RefundSeriaizer(refund)
                    return Response(response_serializer.data, status=status.HTTP_201_CREATED)
                else:
...
def payment_analytics(request):
    '''Аналитика по платежам для админов'''
    ...
    monthly_renevue = monthly_payments.aggregate(
        total=Sum('total')
    )['total'] or 0
    ...
    return Response({
        ...
        'successfull_payment': successful_payments,
        'success_rate': (successful_payments / total_payments * 100) if total_payments > 0 else 0,
        'total_renevue': float(total_revenue),
        'monthly_renevue': float(monthly_renevue),
