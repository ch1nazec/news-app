...
class PinnedPostSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not hasattr(user, 'subscription') or not user.subscription.is_active:
            raise serializers.ValidationError({
                'non_fiel_errors': ['Active subscription required to pin posts.']
            })
        return attrs
    
    def create(self, validated_data):
        """Создаёт закрепленный пост"""
        validated_data['user'] = self.context['request'].user
        return super().create()


class SubsctiptionHistorySerializer(serializers.ModelSerializer):
...
class UserSubscriptionStatusSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        '''Формирует ответ с информацией о подписке'''
        user = instance
...
class PinPostSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        # Check on have subscribe on user
        if not hasattr(user, 'subscription') or not user.subscription.is_active:
            raise serializers.ValidationError({
