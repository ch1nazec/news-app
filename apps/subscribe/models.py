...
class SubscriptionPlan(models.Model):
    ...
    duration_days = models.PositiveIntegerField(max_length=30)
...
class Subscription(models.Model):
    ...
    def days_remaining(self):
        '''Возврат кол-во дней до конца подписки'''
    ...
    def extend_subscriptions(self, days=30):
        '''Продлевает подписку на указанного кол-во дней'''
        if self.is_active:
...
class PinnedPost(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not hasattr(self.user, 'subscriptions') or not self.user.subscription.is_active:
